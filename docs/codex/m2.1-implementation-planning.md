...
> superseded.
>
> **Lifecycle correction (2026-08-05).** Statements below that treat the first compiled
> graph as immutable during planning are historical behavior. The canonical target is
> [`planning-lifecycle.md`](planning-lifecycle.md): planning proposes draft revisions,
> Tasker recompiles and validates them, and only the accepted execution graph is frozen.
