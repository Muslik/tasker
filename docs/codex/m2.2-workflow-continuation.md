...
> **Superseded runtime record.** Late-discovery behavior remains required, but the
> custom parent/child scheduler is replaced by validated graph revisions and Temporal
> Child Workflows where lifecycle separation warrants them.
>
> Planning-time changes no longer use this continuation model in the canonical target.
> They revise the pre-execution draft as described in
> [`planning-lifecycle.md`](planning-lifecycle.md). This document remains evidence for
> discoveries made after execution freeze.
