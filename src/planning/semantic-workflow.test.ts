...
import { compileSemanticWorkflow } from '../graph/index.js';
...
describe('semantic workflow harness', () => {
  ...
  it('freezes task-caused CI and human-review repair inside the delivery feedback loop', () => {
    ...
    expect(
      validateWorkflowObligations(result.value.compiled.graph, { origin: 'fixture' }).issues,
    ).toEqual([]);
  });

  it('rejects delivery preparation inside the review feedback loop', () => {
    const taskInput = {
      objective: 'Fix the flight card layout and publish it',
      repository: 'onetwotrip/front-avia',
      taskId: 'AVIA-1',
    ...
    const result = compileSemanticWorkflow({
      ...
      source: {
        schemaVersion: 1,
        id: 'unsafe-review-delivery-preparation',
        version: 1,
        root: {
          kind: 'sequence',
          id: 'task-work',
    ...
    if (!result.ok) throw new Error(JSON.stringify(result.error));
    const report = validateWorkflowObligations(result.value.compiled.graph, { origin: 'fixture' });
