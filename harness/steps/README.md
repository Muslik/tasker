...
use to assemble a task-specific workflow. One JSON file declares one versioned step.
...
- `executor.prompt`: the readable instruction file;
...
Removing a manifest removes that capability from future run snapshots. Existing frozen runs
keep their immutable snapshot and do not observe later harness edits.
